...
function M.mod(modname)
	...
	if not chunk then
		error("splatter.util.mod: could not load module at " .. path .. ": " .. err)
	end
...
function M.invert(color)
	...
	elseif type(color) == "string" then
		local hsluv = require("splatter.hsluv")
		if color ~= "NONE" then
...
---@param groups splatter.Highlights
...
function M.cache.file(key)
	return vim.fn.stdpath("cache") .. "/splatter-" .. key .. ".json"
end

---@param key string
function M.cache.read(key)
	---@type boolean, splatter.Cache
